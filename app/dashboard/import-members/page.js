...
export default function ImportMembersPage() {
  ...
  const previewMutation = useMutation({
    mutationFn: async (file) => {
      ...
      const response = await fetch('/api/members/import', { // ← SAME API
        method: 'POST',
        credentials: 'include',
        body: formData
      });
      ...
      {uploadProgress && (
        <div className={styles.progressCard}>
          <div className={styles.progressHeader}>
            <h3>
              {uploadProgress.stage === 'analyzing' && '📊 Analyzing Excel file...'}
              {uploadProgress.stage === 'validating' && '✅ Running validations...'}
              {uploadProgress.stage === 'importing' && '📥 Importing members...'}
              {uploadProgress.stage === 'processing' && '⚙️ Creating accounts...'}
            ...
            <div className={styles.summaryCard}>
              ...
              {!previewData.validation.summary.canImport && (
                <div className={styles.errorBanner}>
                  ❌ Cannot import: Fix all errors and duplicates first, then re-upload
                </div>
              )}

              {previewData.validation.summary.canImport && (
                <div className={styles.successBanner}>
                  ✅ All validations passed! Ready to import {previewData.validation.summary.valid} members
                </div>
              )}
            </div>
