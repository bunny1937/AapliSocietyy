...
export const memberSchema = z.object({
  ...
  // D1: Aadhaar is no longer accepted on member writes. Stripped rather than
  // rejected — a client still sending it (an old cached bundle, a saved form)
  // should not have its whole update fail, it should simply not have that one
  // field applied. Existing stored values are cleared by
  // scripts/clear-member-aadhaar.mjs; see lib/superadmin/societyRedaction.js
  // for why it never reaches an export either.
  aadhaar: z.undefined().or(z.any().transform(() => undefined)).optional(),
