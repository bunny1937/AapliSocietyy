...
export const SETTINGS = [
  ...
  // What a plan costs per month, in whole rupees. These drive EXPECTED
  // revenue on the platform dashboard; COLLECTED revenue is summed from
  // Society.subscription.paymentHistory, which is real money that actually
  // arrived. The gap between the two is the arrears figure.
  //
  // All four default to 0, and the dashboard treats "every price is 0" as
  // "not configured yet" and shows collected-only rather than rendering a
  // confident ₹0 MRR. Set them once pricing is decided.
  {
    key: "PLAN_PRICE_FREE",
    env: "PLAN_PRICE_FREE",
    type: TYPE.INT,
    ...
    min: 0,
    max: 1000000,
    group: "Subscription — plan prices",
    label: "Free plan (₹/month)",
    why: "Normally 0. Present so the plan table is complete and the dashboard never has to special-case one plan.",
  },
  {
    key: "PLAN_PRICE_BASIC",
    env: "PLAN_PRICE_BASIC",
    type: TYPE.INT,
    fallback: 0,
    min: 0,
    max: 1000000,
    ...
    why: "Monthly price billed to a society on the Basic plan. Expected MRR counts this for every Active society on Basic; societies on Trial are excluded until they convert.",
  },
  {
    key: "PLAN_PRICE_PREMIUM",
    env: "PLAN_PRICE_PREMIUM",
    type: TYPE.INT,
    fallback: 0,
    min: 0,
    max: 1000000,
    ...
    why: "Monthly price billed to a society on the Premium plan.",
  },
  {
    key: "PLAN_PRICE_ENTERPRISE",
    env: "PLAN_PRICE_ENTERPRISE",
    type: TYPE.INT,
    fallback: 0,
    min: 0,
    max: 1000000,
    ...
    why: "Monthly price billed to a society on the Enterprise plan.",
  },
