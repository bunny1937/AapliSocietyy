...
import { listActiveStaffRoleKeys } from "@/lib/rbac/assignment-service";
...
export async function issueTokens(user, profile) {
  ...
  const claims = {
    ...
    kind: profile?.kind ?? "Residential",
    shopId: profile?.shopId ? String(profile.shopId) : undefined,
    ...
    // Shared with the refresh token below so /v1/auth/logout's
    // revoked:jti:<jti> denylist (middleware.js reads it on every request)
    // actually has something to revoke — access tokens didn't carry a jti
    // before this, so logout silently revoked nothing for mobile clients.
    jti,
  };
  // RBAC roles (Manage Access) live in RoleAssignment, keyed on {userId,
  // societyId} — completely separate from profiles[]. Without this, a
  // Clubhouse_manager grant never reaches the mobile app: issueTokens used to
  // only look at profile.role/user.role, so the app kept showing whatever
  // profile (e.g. the linked shop) was already on the account.
  if (claims.societyId) {
    claims.staffRoles = await listActiveStaffRoleKeys(claims.userId, claims.societyId);
  }
...
export async function reissueTokens(oldClaims, user) {
  ...
  const claims = {
    ...
    kind: oldClaims.kind ?? "Residential",
    shopId: oldClaims.shopId,
    activeProfileId: oldClaims.activeProfileId,
    occupancyType: oldClaims.occupancyType,
    mustChangePassword: user.mustChangePassword === true || undefined,
    jti,
  };
  // Same reasoning as issueTokens: refresh the RBAC role snapshot so a
  // password-change reissue doesn't carry a stale staffRoles list.
  if (claims.societyId) {
    claims.staffRoles = await listActiveStaffRoleKeys(claims.userId, claims.societyId);
  }
