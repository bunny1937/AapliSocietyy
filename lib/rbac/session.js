...
import { setSessionEpochFloor } from "@/lib/session-epoch";
...
export async function bumpSessionEpoch(userId) {
  ...
  const epoch = updated?.sessionEpoch ?? null;
  // middleware.js (edge-safe, no Mongo access) enforces this floor on every
  // /api/* request and protected-page load for BOTH RBAC and legacy
  // lib/authz.js-guarded routes — sessionEpoch is the only place a role
  // reduction or suspendUser() ever mutates, so this one cache write closes
  // the staleness gap everywhere at once instead of touching the 100+ route
  // files that call requireAuth()/requireRoles().
  await setSessionEpochFloor(userId, epoch);
...
export async function bumpRbacVersion(societyId) {
  await connectDB();
  // societyId here is always the Society _id (that's what RoleAssignment/Role/
  // User.societyId carry — see permission-engine.js). Filtering by the
  // `societyId` FIELD (the human-readable code, e.g. "green_valley_...") never
  // matched an _id string, so rbacVersion silently never incremented and the
  // 300s perms cache never invalidated on grant/revoke/role-edit.
  if (!mongoose.isValidObjectId(societyId)) return;
