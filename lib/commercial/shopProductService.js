...
export async function createOwnerProduct({ societyId, shopId, userId, input }) {
  ...
  try {
    ...
    if (isDuplicateKeyError(err)) throw duplicateNameError();
    throw err;
  }
}
...
export async function listMemberProducts({ societyId, shopId, q, categoryId, page, pageSize }) {
  ...
  const [rows, total] = await Promise.all([
    ShopProduct.find(filter)
      .select("name description unitLabel price categoryId imageKey trackStock quantity reservedQuantity lowStockThreshold minOrderQty maxOrderQty sortOrder")
