...
export function computeOpenState(shop, now = new Date()) {
  ...
  if (!hasWeekly && !hasOverrideToday) {
    return { state: OPEN_STATES.UNKNOWN, timezone, label: "Hours not listed", opensAt: null, closesAt: null };
  }
...
export function toShopOwnerDto(shop, { now = new Date() } = {}) {
  ...
  if (storefront.pickupEnabled !== true && storefront.deliveryEnabled !== true) {
