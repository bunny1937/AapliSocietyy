...
export class BillingEngine {
  ...
  applyInterest(principalAmount, interestRate, dueDate, gracePeriodDays) {
    ...
    if (!isDateAfterGracePeriod(dueDate, gracePeriodDays, now)) {
      return 0;
    }
    ...
    const daysOverdue = calculateDaysBetween(graceEndDate, now);

    const monthsOverdue = Math.ceil(daysOverdue / 30);

    const interest = (principalAmount * interestRate * monthsOverdue) / 100;
